...
In the `data-raw` folder is the database.csv file, which consists of the junction of all the stations analyzed from **CFSR**. To create this single file we used the data available in `data-raw/cfsr_new`, which is the data of each station with corrected date (`data-raw/cfsr`). In this folder you will also find the nasa power files (`data-raw/power`).
